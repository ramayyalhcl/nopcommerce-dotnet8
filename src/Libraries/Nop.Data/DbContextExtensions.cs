...
namespace Nop.Data
{
    public static class DbContextExtensions
    {
        ...
        private static T InnerGetCopy<T>(IDbContext context, T currentCopy, Func<Microsoft.EntityFrameworkCore.ChangeTracking.EntityEntry<T>, T> func) where T : BaseEntity
        {
            //Get the database context
            DbContext dbContext = CastOrThrow(context);

            //Get the entity tracking object
            var entry = dbContext.Entry(currentCopy);
        ...
        /// <returns></returns>
        private static Microsoft.EntityFrameworkCore.ChangeTracking.EntityEntry<T> GetEntityOrReturnNull<T>(T currentCopy, DbContext dbContext) where T : BaseEntity
        {
        ...
        {
            // EF Core: Get original values from ChangeTracker
            return InnerGetCopy(context, currentCopy, e => 
            {
                var originalValues = (T)Activator.CreateInstance(typeof(T));
                foreach (var prop in e.OriginalValues.Properties)
                {
                    var originalValue = e.OriginalValues[prop];
                    var propertyInfo = typeof(T).GetProperty(prop.Name);
                    if (propertyInfo != null && propertyInfo.CanWrite)
                        propertyInfo.SetValue(originalValues, originalValue);
                }
        ...
        {
            // EF Core: Reload from database
            return InnerGetCopy(context, currentCopy, e => 
            {
                var dbValues = e.GetDatabaseValues();
                return dbValues?.ToObject() as T;
            });
        }
        ...
        public static void DropPluginTable(this DbContext context, string tableName)
        {
            ...
            //drop the table
            var checkTableSql = "SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = {0}";
            if (context.Database.SqlQueryRaw<int>(checkTableSql, tableName).Any())
            {
        ...
        {
            // Simple implementation - return entity type name
            // EF Core metadata API is different from EF6
            // TODO: Use context.Model.FindEntityType(typeof(T)) if precise table name needed
        ...
        {
            // EF6 metadata not available in EF Core
            // TODO: Reimplement using EF Core IModel metadata if needed
        ...
        {
            // EF6 metadata APIs (IObjectContextAdapter, MetadataWorkspace) don't exist in EF Core
            // EF Core uses IModel and entity type metadata differently
            // TODO: Reimplement using EF Core metadata API if needed
            // For now, return empty dictionary
            return new Dictionary<string, decimal>();
            
            /* EF6 CODE - Not compatible with EF Core
        ...
        /* EF6 METADATA METHOD - Not compatible with EF Core
        ...
        */
        ...
        {
            // EF Core: Get database name from connection string
