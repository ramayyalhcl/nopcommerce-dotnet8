...
// using System.Linq.Dynamic; - Replaced with Core version for .NET 8
...
namespace Nop.Web.Framework.Validators
{
    public abstract class BaseNopValidator<T> : AbstractValidator<T> where T : class
    {
        ...
        protected virtual void SetStringPropertiesMaxLength<TObject>(IDbContext dbContext, params string[] filterPropertyNames)
        {
            ...
            var expression = maxLength.Keys.ToDictionary(name => name, name => DynamicExpressionParser.ParseLambda<T, string>(null, false, name, null));
        ...
        protected virtual void SetDecimalMaxValue<TObject>(IDbContext dbContext)
        {
            ...
            var expression = maxValues.Keys.ToDictionary(name => name, name => DynamicExpressionParser.ParseLambda<T, decimal>(null, false, name, null));
