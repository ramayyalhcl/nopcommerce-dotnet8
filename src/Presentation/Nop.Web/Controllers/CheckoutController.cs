...
namespace Nop.Web.Controllers
{
    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        // [ValidateInput] - removed in .NET 8
        ...
        public virtual ActionResult PaymentInfo()
        {
                ...
                (paymentMethod.PaymentMethodType == PaymentMethodType.Redirection && _paymentSettings.SkipPaymentInfoStepForRedirectionPaymentMethods))
            {
                ...
                //session save - TODO: Use Session with JSON serialization or TempData
                // _httpContext.Session.SetString("OrderPaymentInfo", JsonSerializer.Serialize(paymentInfo));
        ...
        public virtual ActionResult EnterPaymentInfo(IFormCollection form)
        {
            ...
            // TODO: Plugin API changed - payment controllers now use ViewComponent pattern
            ...
            var warnings = new List<string>(); // Stubbed - validation needs ViewComponent implementation
            foreach (var warning in warnings)
                ModelState.AddModelError("", warning);
            if (ModelState.IsValid)
            {
                //get payment info - TODO: Plugin API changed to ViewComponent
                var paymentInfo = new ProcessPaymentRequest(); // paymentController.GetPaymentInfo(form);
                //session save - TODO: Use Session with JSON serialization
                // _httpContext.Session.SetString("OrderPaymentInfo", JsonSerializer.Serialize(paymentInfo));
                return RedirectToRoute("CheckoutConfirm");
            }
        ...
        public virtual ActionResult ConfirmOrder()
        {
            ...
            try
            {
                // TODO: Session storage changed - need JSON deserialization or use TempData
                var processPaymentRequest = null as ProcessPaymentRequest; // _httpContext.Session.GetString("OrderPaymentInfo");
                ...
                {
                    _httpContext.Session.Remove("OrderPaymentInfo"); // Clear session
        ...
        public virtual ActionResult OpcSaveBilling(IFormCollection form)
        {
            try
            {
                ...
                else
                {
                    ...
                    // TryUpdateModel - use await TryUpdateModelAsync( in .NET 8 TryUpdateModel(model.NewAddress, "BillingNewAddress");
                ...
                if (cart.RequiresShipping())
                {
                    ...
                    var model = new CheckoutBillingAddressModel();
                    // TryUpdateModel - use await TryUpdateModelAsync( in .NET 8 TryUpdateModel(model);
        ...
        public virtual ActionResult OpcSaveShipping(IFormCollection form)
        {
            try
            {
                ...
                else
                {
                    //new address
                    var model = new CheckoutShippingAddressModel();
                    // TryUpdateModel - use await TryUpdateModelAsync( in .NET 8 TryUpdateModel(model.NewAddress, "ShippingNewAddress");
        ...
        public virtual ActionResult OpcSavePaymentInfo(IFormCollection form)
        {
            try
            {
                ...
                // TODO: Plugin API changed - payment controllers use ViewComponent pattern
                // var paymentControllerType = paymentMethod.GetControllerType();
                // var paymentController = HttpContext.RequestServices.GetService(paymentControllerType) as BasePaymentController;
                // var warnings = paymentController.ValidatePaymentForm(form);
